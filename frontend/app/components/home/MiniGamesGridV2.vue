      ...
      <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div class="flex items-center gap-2">
          <button
            ...
            class="relative overflow-hidden px-4 py-1.5 rounded-full text-xs md:text-sm transition-colors"
          ...
          >
            ...
            <span
              v-if="active === t.key"
              class="mini-countdown-pill-fill"
        ...
        <div class="flex items-center gap-2 self-start md:self-auto">
          ...
          <NuxtLink class="text-ink-3 hover:text-ink text-xs px-3 py-1.5 border border-line rounded transition-colors" :to="routes[active]">{{ t('action.showAll') }}</NuxtLink>
        </div>
      </div>
