...
- [x] R6(2026-07-15:客戶後台 `/admin` — 終端客戶控制系統,四分頁(UiTabs):
  內容文案(Banner 輪播逐欄編輯:badge/標題/主打字/副標/CTA,即時反映)、促銷管理
  (新增/改名/↑↓排序/移除)、版面區塊(顯示開關 + ↑↓排序)、皮膚(限模板允許清單)。
  可控範圍由模板定義:新增 `config/template.ts`(TemplateClientScope:skins 白名單/
  lockedSections 版面錨點不可隱藏移動/editable 內容範圍),/admin 全程按此授權渲染控制項,
  鎖定區塊顯示 🔒+「鎖定」標記、相鄰移動也不可擠動錨點;變體選擇/新增區塊/匯出模板包
  維持設計師工具(/studio)專屬,客戶後台刻意不提供。
  內容層新增 `stores/content.ts`(banners/promoCards 以 mock 為種子;actions 一律就地變更
  保住元件端 reference 反應鏈;save() 占位),Banner×3 + Promotion×3 六個元件改讀 content store
  (mock/home.ts 降為種子資料)。
  驗證:build + generate 54 路由通過;Playwright e2e 走完整客戶流程 9 檢查全過
  (文案編輯/促銷增刪/鎖定區塊無控制項/不可越過錨點移動/開關/換膚/占位儲存/回站點後
  文案+CTA+隱藏+新促銷+移除促銷+皮膚全部生效)。)
- [x] R7(2026-07-16:《模板開發規範》`docs/template-guide.md` — 四層架構總覽(皮膚/變體/
  組合/內容 + 雙後台)、固定資料夾結構、皮膚建立流程(RGB channel 規則、單一來源)、
  區塊與變體規則(3–5 變體、v1 基準、抽取判斷)、config schema(BLOCKS/SectionConfig/
  site store/TEMPLATE.client)、命名規範、雙後台分工表(/studio=設計師 vs /admin=客戶)、
  出貨驗收清單、starter 使用步驟。
  Starter 範本 `templates/starter/` — 去內容留骨架、可直接運行:皮膚 base(:root)+demo
  兩套、Ui 元件層 ×10 原樣共用、BlockRenderer/utils(zip/themes/studio-draft)原樣共用、
  main.css 只留泛用段(btn/input/card/section-title/pill/text-gradient)、範例區塊
  ExampleHero ×3 變體(示範完整規範)+ ExampleCards ×2 + SiteHeader/SiteFooter chrome、
  config-driven 首頁、`/ui-kit`(皮膚自動清單+變體型錄)、`/studio` 與 `/admin` 全功能、
  中性種子內容(config/mock/content.ts);匯出檔名改讀 TEMPLATE.name(frontend 同步修正)。
  驗證:starter `npm install` + build + generate(12 路由)零錯誤;smoke e2e 4 組全過
  (首頁渲染/ui-kit 皮膚 base→demo 切換+變體型錄/studio 預覽+變體+匯出 zip/admin 鎖定
  hero+文案編輯回站點生效);frontend build 同步重驗通過。)

- [x] 總驗收(2026-07-16:R0–R7 全數完成後,依 template-guide §8 出貨清單回審 WIN100 本體。
  發現並修復:R3 清單外的頁面/元件殘留 Tailwind 內建色 155 處(account 100、sport 22、
  withdrawal 13、其餘 12 檔零星),依 style-guide 既有等值表批次替換
  (white→ink、gray-300/400/500/600→ink-2/3/4、green-400→success、red-400→danger、
  gray-900 on primary→on-primary、gray-800/700→line-soft/line);遮罩類新增 `--c-scrim`
  token(四份皮膚檔 + 兩份 tailwind.config),`bg-black/NN`→`bg-scrim/NN`(彈窗 backdrop、
  圖上標籤、studio 預覽底)。starter 同步修正。
  驗證:frontend + starter build 通過;內建色 grep 歸零(frontend + starter);
  account/withdrawal 雙視口 pixel-diff 0.000%,sport 0.5–0.9%(僅聯賽名稱小字
  gray-400→ink-3 的既定等值映射,裁圖視驗無感差異)。)
