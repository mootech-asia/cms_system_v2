...
- [~] 2 共用元件(部分完成)
  ...
  - [x] **公開頁 topbar 皮膚切換觸發 `.skin-switch`(20×)+ 登入/註冊 CTA `.topbar-cta`(+--primary/--secondary,20×)**
    ,hook(aria-label="Switch skin")保留,parity real-diff 0、9/9、28/28。
  - [x] **FilteredGrid 遊戲卡 `.fg-card`(+-media/-img/-title/-provider)**(hot-games.html「全部遊戲」格 30 張;
    保留 group/group-hover:scale-105、p-4、btn-primary)parity real-diff 0、9/9、28/28。
  - [ ] 大廳 app-header 內層剩餘(帳號列/分類 nav 容器等 — 多為可讀 layout,視需要)。
