...
>
> 🎯 **業主目標狀態(2026-07-23 親口確認,寫入 DNA)**
>
> **未來開發流程**:工廠(= 本 repo `cms_system_v2`)選好版型 → 定案 → 轉出「設計 HTML+CSS+JS」
> (只含交付的前台/後台,**不含工廠本身**)→ 業主再做一次「工程師要的編譯」→ 成為工程師要的結果
> → 新建 repo + 建立版本號 → 設計+程式碼轉交工程師接手後續串接。(即:工廠常駐本 repo;每個定案
> 版型 export 成獨立交付專案,如 `cms_system_v3`。)
>
> **v2 / v3 都要收斂成純 HTML+CSS+JS 乾淨且「手寫可讀」狀態**,三件套齊全:① 前台 ② 設計後台(`/studio`)
> ③ **客戶後台(`/admin`)—— v3 也要有(新需求,原判定 v3 無 admin 已作廢)**。對應路線圖:
> #3 前台去編譯化 Stage B 語意化、#4 studio 靜態化取代 Nuxt/Vite、#5 客戶後台靜態化(v2+v3)、
> #6 移除 `frontend/`/`vue3-app/` + 編譯 build 步驟。**另:v3 的 bug 要一併處理乾淨。**
